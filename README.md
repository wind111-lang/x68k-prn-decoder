...
XM6が保存したX68000用SHARP形式のPRNファイルを解析し、UTF-8テキストと印刷用HTMLへ変換するPHP CLIツールです。

『Super X-Day X'95』の出力で使われるJIS X 0208文字列とプリンタ制御コード（`ESC K`、`ESC H`、`FS S`、改ページ）に対応しています。PRN内のデータを実行することはなく、ローカルでバイト列として解析します。
